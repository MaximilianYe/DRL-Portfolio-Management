...
def calculate_technical_indicators(df, rsi_window=14, bb_window=20, base_date="2021-01-01"):
    ...
    df['price_vs_ma252'] = df['close'] / df['close'].rolling(252).mean() - 1
    ...
    df['volume_momentum_5d'] = df['volume'].pct_change(5).fillna(0)
    ...
    if not df.empty:
        ...
        df['amplitude'] = df['amplitude'].fillna(first_day_amplitude)
    else:
    ...
    df['volatility_22d'] = df['daily_return'].rolling(window=22).std() * np.sqrt(252)
