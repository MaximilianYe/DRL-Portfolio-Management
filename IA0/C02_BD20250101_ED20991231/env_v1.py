...
class SimpleUSStockEnv(gym.Env): 
    def __init__(self, stock_data_dict, stock_features=['price_vs_ma252', 'momentum_10d_percentile', 'price_percentile_60', 'RSI', 'volatility_22d', 'volume_momentum_5d', 'momentum_5d'], #'vc_5d_mean', 
       ...
       # Observation dimension = total features + position weights + 2 macro features
        obs_dim = total_features + self.n_stocks + 1 + 2
    ...
    def _get_observation(self):
        ...
        first_stock = self.stock_names[0]
        first_stock_df = self.stock_data_dict[first_stock]
        
        macro_features = [
            first_stock_df['market_uptrend'].iloc[self.current_step],
            first_stock_df['market_volatility'].iloc[self.current_step]
        ]
        observations.append(np.array(macro_features))
